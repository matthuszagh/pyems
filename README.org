...
** TODO add convenience structures
These will be collections of primitives that are frequently needed,
such as vias, a PCB stackup, a microstrip or GCPW line (adds grounding
to the port), etc but are annoying to build out of CSXCAD primitives
(for instance vias...). This will rely heavily on priorities, which is
a primary reason the automesh needs to support it. This should present
the user with some non-confusing way of setting priorities.

** TODO display PML
Add network option to display PML. Should be implemented as an air
primitive with negative priority.

** TODO PML warning for probes / feeds / nf2ff box
Mesh / structure generation should emit a warning (error?) when a
probe/feed/nf2ff box (what about field dump boxes?) are placed in a
PML. As far as I can tell this is never the correct behavior.

There should also be a way to list every primitive that overlaps the
PML, even if the overlap is innocuous.
